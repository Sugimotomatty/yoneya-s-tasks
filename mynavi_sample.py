...
def main():
    ...
    #ページネイション内の遷移するurlを取得する
    #この状態やとnoneしかない、理由は階層構造になってるから。これやとクラス内にあるhrefタグしか取得できない
    #これが成功したら次にやることはこれを関数にして外に出す、それから下のいろいろ取得したやつも関数にする
    #そしてそれをfor文で回して2ページ以降も取得していく
    #オプション①はわりとすぐできると思う
    #78~87同じこと二回書いてしまっているから良くない
    #ここでurlを全て取得している
    css_selector="body > div.wrapper > div:nth-child(5) > form > div > nav:nth-child(51) > ul > li:nth-child({}) > a"
    url_css=driver.find_element(by=By.CSS_SELECTOR, value=css_selector.format(2))
    url = url_css.get_attribute("href")
    hrefList=[]
    hrefList.append(url)
    num=3
    try:
        while len(hrefList)==num-2:
            url_css=driver.find_element(by=By.CSS_SELECTOR, value=css_selector.format(num))
            url = url_css.get_attribute("href")
            hrefList.append(url)
            num+=1
            if num>=5:
                time.sleep(2)
                driver.execute_script("arguments[0].click();", url_css)
    except:
        pass
